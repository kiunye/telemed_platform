defmodule TelemedCore.Accounts.Session do
  ...
  policies do
    # Allow unauthenticated session lookup by refresh token for token refresh
    bypass action(:read) do
      authorize_if(always())
    end

    # Allow unauthenticated session creation during login
    bypass action(:create_session) do
      authorize_if(always())
    end
